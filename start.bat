...
if not defined PYTHON_BASE_IMAGE set "PYTHON_BASE_IMAGE=docker.1ms.run/python:3.11-slim"
if not defined NODE_BASE_IMAGE set "NODE_BASE_IMAGE=docker.1ms.run/node:20-alpine"
if not defined NGINX_BASE_IMAGE set "NGINX_BASE_IMAGE=docker.1ms.run/nginx:1.27-alpine"
if not defined PIP_INDEX_URL set "PIP_INDEX_URL=https://pypi.org/simple"
if not defined APT_MIRROR_HOST set "APT_MIRROR_HOST=mirrors.aliyun.com"
...
if not defined THEME_PROVISIONAL_AUTO_DECISION_ENABLED set "THEME_PROVISIONAL_AUTO_DECISION_ENABLED=true"
if not defined THEME_PROVISIONAL_DECISION_THRESHOLD set "THEME_PROVISIONAL_DECISION_THRESHOLD=0.60"
...
if exist "%COMPOSE_FILE%" goto check_prod_compose_ok
echo [ERROR] compose.prod.yml was not found.
exit /b 1
:check_prod_compose_ok

if exist "%TEST_COMPOSE_FILE%" goto check_test_compose_ok
echo [ERROR] compose.test.yml was not found.
exit /b 1
:check_test_compose_ok
...
powershell -NoProfile -Command ^
  ...
  "$content += 'THEME_PROVISIONAL_AUTO_DECISION_ENABLED=' + $env:THEME_PROVISIONAL_AUTO_DECISION_ENABLED;" ^
  "$content += 'THEME_PROVISIONAL_DECISION_THRESHOLD=' + $env:THEME_PROVISIONAL_DECISION_THRESHOLD;" ^
...
if errorlevel 1 (
  echo [ERROR] Failed to write Docker compose env file.
  exit /b 1
)
exit /b 0
