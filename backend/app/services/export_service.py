...
class ExportService:
    ...
    async def request_export_album(self, album_id: str, user_id: str | None, format: str = "html") -> dict | None:  # noqa: A002
        ...
        return await self.task_service.request_task(
            ...
            task_params={"format": format},
            idempotency_key=f"export:{album_id}:{format}:{album.render_revision}",
        ...
        try:
            ...
            if format == "pdf":
                ...
                try:
                    ...
                    stored = await self.storage.save_export(
                        album_id,
                        output_path.name,
                        file_bytes,
                        "application/pdf",
            ...
            await self.task_service.complete_success(
                ...
                metrics_payload={"duration_ms": round((perf_counter() - started) * 1000), "format": export_format},
                result_revision=album.render_revision,
            )
            await self.session.commit()
            return {"export_id": export.id, "format": export_format, "warnings": warnings}
