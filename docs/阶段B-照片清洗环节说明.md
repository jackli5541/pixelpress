...
这类结果仍然只是 `suggestion=remove`，默认不会从后续流程中排除。
...
> 照片属于 SHA-256 完全相同的副本子组，并且不是该子组排序最靠前的规范副本，同时该相册命中灰度范围，自动排除模式为 `exact_only`。
...
| `CLEANING_AUTO_EXCLUDE_MODE` | `exact_only` | `off` 关闭自动排除；`exact_only` 仅排除精确副本 |
...
8. 两个严重质量问题且总分低于 3，应建议移除，但仍保留在后续流程中直到人工确认。
