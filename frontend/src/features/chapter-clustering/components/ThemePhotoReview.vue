...
const reasonLabels: Record<string, string> = {
  ...
  calibration_missing: '缺少主题相关性标定，需人工确认',
  calibration_disabled: '主题相关性标定尚未启用，需人工确认',
  calibration_model_mismatch: '标定版本与当前向量模型不匹配',
...
<template>
  <div>
    ...
    <div v-if="assessments.length" class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <article v-for="assessment in assessments" :key="assessment.photo.id" class="overflow-hidden rounded-lg border border-[rgba(79,59,42,0.14)] bg-white">
        <div class="relative">
    ...
    <div v-else class="mt-5 rounded-lg border border-dashed border-[rgba(79,59,42,0.2)] px-4 py-10 text-center text-sm text-[#65584e]">{{ view === 'removed' ? '没有已移出的照片。' : view === 'review' ? '没有待处理的照片。' : '没有已保留的照片。' }}</div>
